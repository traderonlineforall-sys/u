...
export async function findDeviceNicknameMatch(supabase, fp, options = {}){
  ...
  const minSupportingObservations = Math.max(1, Number(
    options.minSupportingObservations ?? DEVICE_AUTO_MIN_SUPPORTING_OBSERVATIONS
  ));
  ...
  try{
    ...
    if(!best.exact_observation && Number(best.supporting_observations || 0) < minSupportingObservations){
...
export async function recordDeviceNickname(supabase, userId, displayName, fp, confidenceScore = 100, options = {}){
  ...
  try{
    const source = String(options.source || "unverified_session");
    ...
    // Learning a recovery fingerprint is separate from promoting owner trust.
    // An explicit first registration/selection or a valid server credential
    // may store the passive profile even when can_strengthen remains false.
    const canLearnFingerprint = canStrengthen || options.learnFingerprint === true || [
      "verified_device_credential",
      "verified_local_device_secret",
    ...
    const observation = await logDeviceObservation(supabase, {
      ...
      user_id:uid,
      source,
      truth_level:canStrengthen
        ? "verified_device"
        : (canLearnFingerprint ? "confirmed_device_binding" : (options.truth_level || "derived_decision")),
