...
// Device nickname recovery rules:
// 105+ = strong enough for automatic login.
// Less than 105 stays manual so the user can choose from the first two suggestions.
export const DEVICE_AUTO_RECOVERY_THRESHOLD = 105;
export const DEVICE_AMBIGUITY_GAP = 18;
export const DEVICE_SUGGESTION_THRESHOLD = 60;
export const DEVICE_MAX_SUGGESTIONS = 2;
...
export const DEVICE_MANUAL_RECOVERY_THRESHOLD = 1;
export const DEVICE_MANUAL_AMBIGUITY_GAP = 0;
export const DEVICE_MANUAL_SELECTABLE_TOP_N = 2;
...
function buildSignalStrings(request, rawSignals){
  ...
  return { sig, ip, ua, lang, platform, screen, viewport, timezone, graphics, canvas, fonts, clientHints, media, storage, audio, keyboard, capabilities, network, battery, mediaDevices, deviceInstance, all };
}
...
export function buildDeviceConfidence(request, rawSignals, secret){
  ...
  const component_hashes = {
    ...
    // High-entropy browser-local key. Unlike IP/UA/canvas signals this value is
    // random per browser and is the only signal allowed to recover a trusted
    // identity without an existing signed cookie.
    device_key_hash: parts.deviceInstance ? hmac(fpSecret, `device-key|${parts.deviceInstance}`) : "",
  };
  ...
  return { ok:true, device_hash, device_key_hash:component_hashes.device_key_hash, component_hashes, signal_summary };
}
...
export async function verifyDeviceNicknameChoice(supabase, fp, selectedUserId, options = {}){
  ...
  const selectableTopN = Math.max(1, Math.min(2, Number(options.selectableTopN || DEVICE_MANUAL_SELECTABLE_TOP_N)));

  const result = await findDeviceNicknameMatch(supabase, fp, {
    threshold: Number(options.autoThreshold || DEVICE_AUTO_RECOVERY_THRESHOLD),
    suggestionThreshold: Math.max(1, Number(options.suggestionThreshold || 1)),
    maxSuggestions: Math.max(selectableTopN, Number(options.maxSuggestions || selectableTopN)),
  });
  ...
  const topSelectable = ranked.slice(0, selectableTopN);
  const chosenIndex = topSelectable.findIndex((x)=>x.user_id === uid);
  const chosen = chosenIndex >= 0 ? topSelectable[chosenIndex] : null;

  if(!chosen){
    return {
      ok:true,
      matched:false,
      reason:"choice_not_in_top_two",
      suggestions: ranked.slice(0, selectableTopN),
      manual_threshold: manualThreshold,
      selectable_top_n: selectableTopN,
    };
  }
