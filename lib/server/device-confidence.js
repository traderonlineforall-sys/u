...
export const DEVICE_AUTO_RECOVERY_THRESHOLD = 94;
export const DEVICE_AMBIGUITY_GAP = 12;
export const DEVICE_SUGGESTION_THRESHOLD = 68;
export const DEVICE_MAX_SUGGESTIONS = 3;
export const DEVICE_MANUAL_RECOVERY_THRESHOLD = 78;
export const DEVICE_MANUAL_SELECTABLE_TOP_N = 3;
export const DEVICE_MIN_EVIDENCE_WEIGHT = 48;
export const DEVICE_AUTO_MIN_STRONG_GROUPS = 3;
export const DEVICE_AUTO_MIN_SUPPORTING_OBSERVATIONS = 2;
export const DEVICE_AUTO_TRUSTED_MATCH_COUNT = 3;
export const DEVICE_AUTO_SINGLE_OBSERVATION_THRESHOLD = 97;
export const DEVICE_MANUAL_MIN_STRONG_GROUPS = 2;
export const DEVICE_MAX_STABLE_CONTRADICTIONS = 1;
...
function scoreCandidate(candidate = {}, componentHashes = {}){
  ...
  const reliability = Math.min(2.2, Math.log2(Math.max(1, Number(candidate.match_count || 0)) + 1) * 0.65);
  const penalty = stableContradictions * 8;
  const score = Math.max(0, Math.min(100, base + reliability + recencyBonus(candidate.last_seen_at) - penalty));
...
function aggregateByUser(rows = []){
  ...
  for(const group of map.values()){
    ...
    const supportBonus = Math.min(2.5, Math.max(0, support.length - 1) * 0.8);
    result.push({
      ...best,
      user_id:group.user_id,
      score:Math.min(100, Math.round((Number(best.score || 0) + supportBonus) * 10) / 10),
      observation_count:group.observations.length,
      supporting_observations:support.length,
      max_match_count:Math.max(...group.observations.map((item)=>Number(item.match_count || 0)), 0),
    });
  }
...
async function buildNicknameSuggestions(supabase, scored = [], minScore = DEVICE_SUGGESTION_THRESHOLD, maxItems = DEVICE_MAX_SUGGESTIONS){
  ...
  for(const item of scored){
    ...
    if(Number(item.evidence_weight || 0) < Math.min(DEVICE_MIN_EVIDENCE_WEIGHT, 42)) continue;
    if(Number(item.stable_contradictions || 0) > DEVICE_MAX_STABLE_CONTRADICTIONS) continue;
...
export async function findDeviceNicknameMatch(supabase, fp, options = {}){
  ...
  const minEvidence = Number(options.minEvidence || DEVICE_MIN_EVIDENCE_WEIGHT);
  const minStrongGroups = Number(options.minStrongGroups || DEVICE_AUTO_MIN_STRONG_GROUPS);
  const maxContradictions = Number(options.maxContradictions ?? DEVICE_MAX_STABLE_CONTRADICTIONS);
  ...
  try{
    ...
    const diagnostics = {
      ...
      max_match_count:Number(best?.max_match_count || 0),
    ...
    const hasRepeatedHistory = Number(best.supporting_observations || 0) >= DEVICE_AUTO_MIN_SUPPORTING_OBSERVATIONS;
    const hasTrustedSingleHistory = Number(best.max_match_count || 0) >= DEVICE_AUTO_TRUSTED_MATCH_COUNT
      && Number(best.score || 0) >= DEVICE_AUTO_SINGLE_OBSERVATION_THRESHOLD
      && Number(best.strong_groups || 0) >= 4
      && Number(best.stable_contradictions || 0) === 0;
    if(!best.exact_observation && !hasRepeatedHistory && !hasTrustedSingleHistory){
...
export async function verifyDeviceNicknameChoice(supabase, fp, selectedUserId, options = {}){
  ...
  const minEvidence = Math.max(1, Number(options.minEvidence || DEVICE_MIN_EVIDENCE_WEIGHT));
  ...
  const result = await findDeviceNicknameMatch(supabase, fp, {
    ...
    maxContradictions:DEVICE_MAX_STABLE_CONTRADICTIONS,
  });
  ...
  if(Number(chosen.stable_contradictions || 0) > DEVICE_MAX_STABLE_CONTRADICTIONS){
