...
export const DEVICE_AUTO_RECOVERY_THRESHOLD = 100;
export const DEVICE_AMBIGUITY_GAP = 18;
export const DEVICE_SUGGESTION_THRESHOLD = 60;
export const DEVICE_MAX_SUGGESTIONS = 3;
// Step 4.6: suggestions are helpful, but selecting a suggestion must not
// be enough to take over another nickname. Manual recovery needs a stronger
// confidence score and must not be ambiguous.
export const DEVICE_MANUAL_RECOVERY_THRESHOLD = 90;
export const DEVICE_MANUAL_AMBIGUITY_GAP = 15;
export const DEVICE_MANUAL_STRONG_OVERRIDE_THRESHOLD = 999;
...
// Step 4.7: passive bonus signals are added on top of the 100% base score.
// They are deliberately low-weight helpers, not primary identity proof.
const BONUS_FIELDS = [
  "keyboard_hash",
  "capabilities_hash",
  "network_hash",
...
const BONUS_WEIGHTS = {
  ...
  network_hash: 2,
  battery_hash: 2,
  media_devices_hash: 2,
};

const DEVICE_SELECT_BONUS = "user_id, display_name, device_hash, ip_hash, ua_hash, lang_hash, platform_hash, screen_hash, timezone_hash, graphics_hash, canvas_hash, fonts_hash, client_hints_hash, media_hash, viewport_hash, storage_hash, audio_hash, keyboard_hash, capabilities_hash, network_hash, battery_hash, media_devices_hash, confidence_score, last_seen_at, revoked_at";
...
function buildSignalStrings(request, rawSignals){
  ...
  // Step 5.6: bind silent recovery to a browser-local device secret when available.
  // This prevents a different PC with a similar fingerprint from silently receiving another nickname.
  const all = [ip, ua, lang, platform, screen, timezone, graphics, canvas, fonts, deviceInstance].join("||");
  return { sig, ip, ua, lang, platform, screen, viewport, timezone, graphics, canvas, fonts, clientHints, media, storage, audio, keyboard, capabilities, network, battery, mediaDevices, all };
}
...
export function buildDeviceConfidence(request, rawSignals, secret){
  ...
  const component_hashes = {
    ...
    keyboard_hash: hmac(fpSecret, `keyboard|${parts.keyboard}`),
    capabilities_hash: hmac(fpSecret, `capabilities|${parts.capabilities}`),
    network_hash: hmac(fpSecret, `network|${parts.network}`),
    battery_hash: hmac(fpSecret, `battery|${parts.battery}`),
    media_devices_hash: hmac(fpSecret, `media-devices|${parts.mediaDevices}`),
  };
  const device_hash = hmac(fpSecret, `device-v3|${parts.all}`);
  const signal_summary = {
    confidence_version: "4.7",
...
function scoreCandidate(candidate = {}, componentHashes = {}){
  ...
  for(const [field, weight] of Object.entries(WEIGHTS)){
    ...
    if(a && b && a === b) baseScore += weight;
  }
  // If the stable Step 4.3 device hash is exact, treat the base identity score as 100
  // and then add only the passive bonus signals that also match.
  if(candidate.device_hash && candidate.device_hash === componentHashes.device_hash){
    baseScore = Math.max(baseScore, 100);
  }
  let bonusScore = 0;
  for(const [field, weight] of Object.entries(BONUS_WEIGHTS)){
    const a = String(candidate[field] || "");
    const b = String(componentHashes[field] || "");
    if(a && b && a === b) bonusScore += weight;
  }
  return Math.max(0, Math.min(110, baseScore + bonusScore));
}
...
async function queryExactDevices(supabase, fp){
  let res = await supabase
    ...
    .select(DEVICE_SELECT_BONUS)
    .eq("device_hash", fp.device_hash)
    .is("revoked_at", null)
    .order("last_seen_at", { ascending:false })
    .limit(3);
  if(res.error && isBonusColumnMissing(res.error)){
    const enhanced = await supabase
      ...
      .order("last_seen_at", { ascending:false })
      .limit(3);
    return { res: enhanced, enhanced:true, bonus:false };
  }
  if(res.error && isEnhancedColumnMissing(res.error)){
    const legacy = await supabase
      ...
      .order("last_seen_at", { ascending:false })
      .limit(3);
    return { res: legacy, enhanced:false, bonus:false };
  }
  return { res, enhanced:true, bonus:true };
}

async function queryFuzzyDevices(supabase, component, enhanced, bonus = false){
  const fields = enhanced ? [...LEGACY_FIELDS, ...ENHANCED_FIELDS, ...(bonus ? BONUS_FIELDS : [])] : LEGACY_FIELDS;
  const orParts = [];
  for(const field of fields){
    const v = String(component[field] || "");
    if(v) orParts.push(`${field}.eq.${v}`);
  }
  const select = bonus ? DEVICE_SELECT_BONUS : (enhanced ? DEVICE_SELECT_ENHANCED : DEVICE_SELECT_LEGACY);
  let q = supabase
    .from("support_user_devices")
    .select(select)
    .is("revoked_at", null)
...
export async function findDeviceNicknameMatch(supabase, fp, options = {}){
  ...
  try{
    ...
    if(candidates.length === 0){
      ...
      let fuzzy = await queryFuzzyDevices(supabase, component, enhanced, bonus);
      if(fuzzy.error && bonus && isBonusColumnMissing(fuzzy.error)){
        bonus = false;
        fuzzy = await queryFuzzyDevices(supabase, component, true, false);
      }
      if(fuzzy.error && enhanced && isEnhancedColumnMissing(fuzzy.error)){
        enhanced = false;
        bonus = false;
        fuzzy = await queryFuzzyDevices(supabase, component, false, false);
      }
      if(fuzzy.error){
    ...
    // Step 5.4.2: automatic nickname recovery is allowed only when the current
    // device hash exactly matches a learned device. Fuzzy matches are useful for
    // suggestions, but they must never silently assign somebody else's nickname.
    // This prevents two similar PCs on the same network from becoming the same user.
    if(usedFuzzyCandidates){
      return {
        ok:true,
        matched:false,
    ...
    if(secondDifferentUser && (best.score - secondDifferentUser.score) < DEVICE_AMBIGUITY_GAP){
      return {
        ...
        second_score:secondDifferentUser.score,
        threshold,
        enhanced,
        bonus,
        suggestions,
    ...
    if(!profile || !displayName){
      return { ok:true, matched:false, reason:"profile_reset_or_missing", best_score:best.score, threshold, enhanced, suggestions };
    }

    return { ok:true, matched:true, user_id:profile.user_id, display_name:displayName, confidence_score:best.score, device_hash:best.device_hash, enhanced, bonus, suggestions };
  }catch(err){
...
export async function verifyDeviceNicknameChoice(supabase, fp, selectedUserId, options = {}){
  ...
  const manualThreshold = Number(options.minScore || DEVICE_MANUAL_RECOVERY_THRESHOLD);
  const manualGap = Number(options.gap || DEVICE_MANUAL_AMBIGUITY_GAP);
  const strongOverride = Number(options.strongOverride || DEVICE_MANUAL_STRONG_OVERRIDE_THRESHOLD);

  const result = await findDeviceNicknameMatch(supabase, fp, {
    threshold: 101,
    suggestionThreshold: Number(options.suggestionThreshold || DEVICE_SUGGESTION_THRESHOLD),
    maxSuggestions: Number(options.maxSuggestions || 8),
  });

  if(!result.ok) return result;
  const suggestions = Array.isArray(result.suggestions) ? result.suggestions : [];
  const ranked = suggestions
    .map((x)=>({ ...x, user_id: normalizeUserId(x.user_id), score: Number(x.score || 0) }))
    .filter((x)=>x.user_id)
    .sort((a,b)=> b.score - a.score);
  const chosen = ranked.find((x)=>x.user_id === uid);
  if(!chosen){
    return { ok:true, matched:false, reason:"choice_not_confident", suggestions, manual_threshold: manualThreshold };
  }

  const chosenScore = Number(chosen.score || 0);
  const best = ranked[0] || null;
  const nextDifferent = ranked.find((x)=>x.user_id !== uid) || null;
  const gapFromNext = nextDifferent ? chosenScore - Number(nextDifferent.score || 0) : 100;
  const isTopCandidate = !!best && best.user_id === uid;
  const strongEnoughToOverrideRank = false; // Step 5.7: manual recovery must be the top candidate; no override bypass.

  // Safety rules:
  // 1) A 60% suggestion may be displayed to help the user remember, but it cannot
  //    directly recover that person's identity. Manual selection starts at 72%.
  // 2) The selected nickname should be the strongest candidate, unless the score is
  //    extremely strong. This prevents someone on a similar device/network from
  //    choosing another visible nickname.
  // 3) If another user is close to the selected score, force a new nickname instead
  //    of risking a wrong identity.
  if(chosenScore < manualThreshold){
    return {
      ok:true,
      matched:false,
      reason:"choice_score_too_low",
      selected_score: chosenScore,
      manual_threshold: manualThreshold,
      suggestions,
    };
  }

  if(!isTopCandidate && !strongEnoughToOverrideRank){
    return {
      ok:true,
      matched:false,
  ...
  if(nextDifferent && gapFromNext < manualGap && !strongEnoughToOverrideRank){
    return {
      ok:true,
      matched:false,
      ambiguous:true,
      ...
      selected_score: chosenScore,
      second_score: Number(nextDifferent.score || 0),
...
function baseDevicePayload(userId, displayName, fp, confidenceScore){
  ...
  return {
    ...
    confidence_score: Math.max(0, Math.min(110, Number(confidenceScore || 110))),
    last_seen_at: now,
    revoked_at: null,
  };
}
...
export async function recordDeviceNickname(supabase, userId, displayName, fp, confidenceScore = 110){
  const uid = normalizeUserId(userId);
  ...
  try{
    ...
    const payload = bonusDevicePayload(uid, name, fp, confidenceScore);
