...
async function validateActiveUser(supabase, userId) {
  ...
  // Older support_users tables have no account-status/reset columns. In that
  // schema there is no suspended state to enforce, so preserve the historical
  // active-user behavior while still requiring the server session registry.
