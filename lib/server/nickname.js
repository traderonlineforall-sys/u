...
function isMissingResetColumn(error){
  const msg = String(error?.message || "");
  return isMissingTableOrColumn(error) && /nickname_reset_required|nickname_locked|account_status|disabled_at|updated_at|created_at/i.test(msg);
}

async function selectProfileWithReset(supabase, userId){
  return supabase
    .from("support_users")
    .select("user_id, display_name, nickname_reset_required, account_status, disabled_at")
    .eq("user_id", userId)
    .limit(1);
}
...
export async function getNicknameProfile(supabase, userId){
  ...
  try{
    ...
    const resetRequired = supportsReset ? !!row?.nickname_reset_required : false;
    const accountStatus = supportsReset ? String(row?.account_status || "active").toLowerCase() : "active";
    const disabled = supportsReset && !!row?.disabled_at;
    ...
    return {
      ...
      active:accountStatus === "active" && !disabled,
...
export async function ensureNicknameForUser(supabase, userId, requestedNickname){
  ...
  if(profile.active === false){
    return { ok:false, status:403, error:"This user profile is not active." };
  }
...
export async function resetNicknameForUser(supabase, userId){
  ...
  try{
    ...
    try{
      ...
      return { ok:false, status:500, error:String(err?.message || err || "Could not revoke device nickname locks.") };
    }

    // Trusted identities/credentials are retained for audit but revoked. A
    // reset never silently transfers the same logical device to another user.
    try{
      const owned = await supabase
        ...
        .eq("user_id", uid)
        .limit(1000);
      if(owned.error && !isMissingTableOrColumn(owned.error)){
        return { ok:false, status:500, error:owned.error.message || "Could not read trusted device identity." };
      }
      const deviceIds = (Array.isArray(owned.data) ? owned.data : []).map((row)=>row?.device_id).filter(Boolean);
      if(deviceIds.length){
        const creds = await supabase
          .from("support_device_credentials")
          .update({ revoked_at:new Date().toISOString() })
          .in("device_id", deviceIds)
          .is("revoked_at", null);
        if(creds.error && !isMissingTableOrColumn(creds.error)){
          return { ok:false, status:500, error:creds.error.message || "Could not revoke trusted device credential." };
        }
      }
      const trusted = await supabase
        .from("support_device_identities")
        .update({ owner_state:"revoked", revoked_at:new Date().toISOString() })
        .eq("user_id", uid)
        .is("revoked_at", null);
      ...
      const sessions = await supabase
        ...
        .eq("user_id", uid)
        .is("revoked_at", null);
      if(sessions.error && !isMissingTableOrColumn(sessions.error)){
        return { ok:false, status:500, error:sessions.error.message || "Could not revoke authenticated sessions." };
      }
    }catch(err){
      return { ok:false, status:500, error:String(err?.message || err || "Could not revoke trusted identity and sessions.") };
    }
