...
async function resolveV2Credential(supabase, parsed, fp) {
  ...
  if (row.revoked_at || row.suspended_at) {
    return { ok: true, matched: false, rejected: true, reason: "credential_inactive" };
  }
  if (Date.parse(row.expires_at || "") <= Date.now()) {
    return { ok: true, matched: false, rejected: true, reason: "credential_expired" };
  }
...
export async function resolveTrustedDeviceIdentity(req, supabase, fp) {
  ...
  if (parsed) {
    const credentialResult = await resolveV2Credential(supabase, parsed, fp);
    const recoverableStaleCredential = credentialResult?.rejected === true && [
      "credential_not_found",
      "credential_expired",
      "credential_device_missing",
      "credential_invalid_or_replayed",
    ].includes(String(credentialResult.reason || ""));
    if (!recoverableStaleCredential) return credentialResult;
    // A stale/invalid credential by itself is not an owner conflict. Continue
    // with the independently stored browser key, which can restore the same
    // owner or classify the browser as a genuinely new device. Explicitly
    // revoked/suspended credentials and owner conflicts still stop above.
  }
  if (token) {
