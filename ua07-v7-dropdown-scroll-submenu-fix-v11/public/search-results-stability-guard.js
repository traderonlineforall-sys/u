(function () {
  ...
  function installDropdownScrollSubmenuSafeV11() {
    var STYLE_ID_V11 = 'ua07-dropdown-scroll-submenu-safe-v11';
    var OLD_STYLE_IDS = [
    ...
    function getDirectSubPanel(sub) {
      if (!sub || !sub.children) return null;
      for (var i = 0; i < sub.children.length; i += 1) {
        var child = sub.children[i];
        if (child && child.classList && child.classList.contains('sub-dropdown-content')) return child;
      }
      return null;
    }

    function hasDirectSubPanel(sub) {
      return !!getDirectSubPanel(sub);
    }
    ...
    function positionSubmenu(sub) {
      if (!sub || !sub.classList) return;
      var panel = getDirectSubPanel(sub);
      if (!panel) return;

      var row = sub.getBoundingClientRect();
      var viewportW = window.innerWidth || document.documentElement.clientWidth || 1366;
      var viewportH = window.innerHeight || document.documentElement.clientHeight || 768;
      var gap = 6;
      var safe = 8;

      var panelBox = panel.getBoundingClientRect();
      var measuredWidth = Math.ceil(panelBox.width || panel.offsetWidth || panel.scrollWidth || 240);
      measuredWidth = clamp(measuredWidth, 180, Math.max(220, viewportW - (safe * 2)));

      var left = Math.round(row.right + gap);
      if (left + measuredWidth > viewportW - safe) {
        left = Math.round(row.left - measuredWidth - gap);
      }
      left = clamp(left, safe, Math.max(safe, viewportW - measuredWidth - safe));

      var wantedHeight = Math.ceil(panel.scrollHeight || panelBox.height || 320);
      var maxHeight = Math.min(420, Math.max(180, viewportH - (safe * 2)));
      var top = Math.round(row.top);
      if (top + Math.min(wantedHeight, maxHeight) > viewportH - safe) {
        top = Math.round(viewportH - safe - Math.min(wantedHeight, maxHeight));
      }
      top = clamp(top, safe, Math.max(safe, viewportH - 80));
      maxHeight = Math.max(160, Math.min(420, viewportH - top - safe));

      sub.style.setProperty('--ua07-submenu-left', left + 'px');
      sub.style.setProperty('--ua07-submenu-top', top + 'px');
      sub.style.setProperty('--ua07-submenu-max-height', maxHeight + 'px');
    }

    function activateSubmenu(sub) {
      if (!sub || !hasDirectSubPanel(sub)) return;
      activeSubmenu = sub;
      sub.classList.add('ua07-submenu-viewport', 'ua07-submenu-open');
      positionSubmenu(sub);
    }

    function deactivateSubmenu(sub) {
      if (!sub || !sub.classList) return;
      sub.classList.remove('ua07-submenu-open', 'ua07-submenu-viewport');
      sub.style.removeProperty('--ua07-submenu-left');
      sub.style.removeProperty('--ua07-submenu-top');
      sub.style.removeProperty('--ua07-submenu-max-height');
      if (activeSubmenu === sub) activeSubmenu = null;
    }

    function scheduleSubmenuPosition() {
      if (!activeSubmenu) return;
      if (rafSubmenu) window.cancelAnimationFrame(rafSubmenu);
      rafSubmenu = window.requestAnimationFrame(function () {
        rafSubmenu = 0;
        if (activeSubmenu && activeSubmenu.classList && activeSubmenu.classList.contains('ua07-submenu-open')) {
          positionSubmenu(activeSubmenu);
        }
      });
    }

    function closestSubDropdown(target) {
      return target && target.closest ? target.closest('.sub-dropdown') : null;
    }

    document.addEventListener('mouseover', function (event) {
      var sub = closestSubDropdown(event.target);
      if (!sub || !hasDirectSubPanel(sub)) return;
      if (event.relatedTarget && sub.contains(event.relatedTarget)) return;
      activateSubmenu(sub);
    }, true);
    ...
    document.addEventListener('focusout', function (event) {
      ...
      window.setTimeout(function () {
        if (!sub.contains(document.activeElement)) deactivateSubmenu(sub);
      }, 0);
    }, true);

    window.addEventListener('resize', scheduleSubmenuPosition, { passive: true });
    window.addEventListener('scroll', scheduleSubmenuPosition, true);

    removeOldDropdownEdits();
    if (document.getElementById(STYLE_ID_V11)) return;

    var style = document.createElement('style');
    style.id = STYLE_ID_V11;
    style.textContent = [
      '/* UA07 V11: internal scroll only + keep nested menus opening. No font/color/padding/background edits. */',
      '.dropdown-content{',
      ...
      '  -webkit-overflow-scrolling: touch !important;',
      '}',
      '.sub-dropdown.ua07-submenu-viewport > .sub-dropdown-content{',
      '  position: fixed !important;',
      '  top: var(--ua07-submenu-top, auto) !important;',
      '  left: var(--ua07-submenu-left, auto) !important;',
      '  right: auto !important;',
      '  bottom: auto !important;',
      '  max-height: var(--ua07-submenu-max-height, min(420px, calc(100vh - 120px))) !important;',
      '  overflow-y: auto !important;',
      '  z-index: 2147483400 !important;',
      '}',
      '.sub-dropdown.ua07-submenu-open > .sub-dropdown-content{',
      '  display: block !important;',
  ...
  installDropdownScrollSubmenuSafeV11();
