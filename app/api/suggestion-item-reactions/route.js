...
function isMissingTableError(error){
  ...
  return code === "42P01" || /relation .*suggestion_item_reactions.* does not exist/i.test(msg) || /Could not find the table/i.test(msg);
}
...
export async function POST(req){
  ...
  try {
    ...
    if (isMissingTableError(error)) {
      return j({ ok: false, missing_table: true, error: "suggestion_item_reactions table is missing" }, { status: 200 });
    }
