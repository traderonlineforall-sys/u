(function(){
  ...
  var LEARN_KEY = "sr_device_confidence_learned_v44";
  ...
  function getCanvasHash(){
    try{
      ...
      ctx.textBaseline="top"; ctx.fillStyle="#f60"; ctx.fillRect(5,5,95,33); ctx.fillStyle="#069"; ctx.font="16px Arial"; ctx.fillText("SR Tool بصمة الجهاز 4.4",12,14);
  ...
  function getAudioHash(){
    return new Promise(function(resolve){
      try{
        var Ctx=window.OfflineAudioContext||window.webkitOfflineAudioContext; if(!Ctx) return resolve("");
        var ctx=new Ctx(1,44100,44100); var osc=ctx.createOscillator(); var comp=ctx.createDynamicsCompressor(); var gain=ctx.createGain();
        osc.type="triangle"; osc.frequency.value=10000; comp.threshold.value=-50; comp.knee.value=40; comp.ratio.value=12; comp.attack.value=0; comp.release.value=0.25; gain.gain.value=0.05;
        osc.connect(comp); comp.connect(gain); gain.connect(ctx.destination); osc.start(0);
        ctx.startRendering().then(function(buffer){
          var data=buffer.getChannelData(0), sample=""; for(var i=4500;i<5000&&i<data.length;i+=7) sample += Math.abs(data[i]).toFixed(7)+",";
  ...
  function getFontsHash(){
    try{
      var c=document.createElement("canvas"), ctx=c.getContext("2d"); if(!ctx) return Promise.resolve("");
      var fonts=["Arial","Tahoma","Times New Roman","Courier New","Verdana","Georgia","Segoe UI","Roboto","Open Sans","Calibri","Cambria","Noto Sans Arabic","Trebuchet MS","Impact","Lucida Console"];
      var txt="SR Tool عقرب الصحراء 0123456789";
      var widths=fonts.map(function(font){ ctx.font="16px "+font+", monospace"; return font+":"+(Math.round(ctx.measureText(txt).width*100)/100); }).join("|");
  ...
  function getWebglInfo(){
    try{
      ...
      var dbg=gl.getExtension("WEBGL_debug_renderer_info"); var vendor=dbg?gl.getParameter(dbg.UNMASKED_VENDOR_WEBGL):gl.getParameter(gl.VENDOR); var renderer=dbg?gl.getParameter(dbg.UNMASKED_RENDERER_WEBGL):gl.getParameter(gl.RENDERER);
      ...
      return Promise.all([hashText(ext.sort().join("|")),hashText(params)]).then(function(h){ return { vendor:String(vendor||""), renderer:String(renderer||""), version:String(gl.getParameter(gl.VERSION)||""), shadingLanguageVersion:String(gl.getParameter(gl.SHADING_LANGUAGE_VERSION)||""), maxTextureSize:String(gl.getParameter(gl.MAX_TEXTURE_SIZE)||""), maxRenderbufferSize:String(gl.getParameter(gl.MAX_RENDERBUFFER_SIZE)||""), maxVertexAttribs:String(gl.getParameter(gl.MAX_VERTEX_ATTRIBS)||""), maxCombinedTextureImageUnits:String(gl.getParameter(gl.MAX_COMBINED_TEXTURE_IMAGE_UNITS)||""), extensionsHash:h[0], paramsHash:h[1] }; });
    }catch{ return Promise.resolve({}); }
  }
  function getClientHints(){
    try{
      var ua=navigator.userAgentData; if(!ua) return Promise.resolve({});
      var base={ brands:Array.isArray(ua.brands)?ua.brands.map(function(b){return b.brand+":"+b.version;}):[], mobile:!!ua.mobile, platform:ua.platform||"" };
      if(typeof ua.getHighEntropyValues==="function") return ua.getHighEntropyValues(["architecture","bitness","model","platformVersion","uaFullVersion","fullVersionList","wow64"]).then(function(high){ return Object.assign(base,{ architecture:high.architecture||"", bitness:high.bitness||"", model:high.model||"", platformVersion:high.platformVersion||"", uaFullVersion:high.uaFullVersion||"", fullVersionList:Array.isArray(high.fullVersionList)?high.fullVersionList.map(function(b){return b.brand+":"+b.version;}):[], wow64:!!high.wow64 }); }).catch(function(){ return base; });
